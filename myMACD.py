...
class MACDApi:
    ...
    def get_temp_close_value(self, path):
        ...
        for i in range(len(time_col)):
            ...
            time_value.append(datetime.datetime(
                temp_time[0], temp_time[1], temp_time[2], temp_time[3], temp_time[4]))
    ...
    def get_close_value(self):
        if self.__var2 == None:
            time_value = filter(self.filter_value_by_time, self.get_temp_close_value(
                'data/'+self.__var1.split('.')[0] + '.xlsx').keys())
            time_value.sort()
            print time_value
            time_close_dict = self.get_temp_close_value(
                'data/'+self.__var1.split('.')[0] + '.xlsx')
            result_list = []
            for item in time_value:
                result_list.append(time_close_dict[item])
            return result_list
        else:
            time_close_dict1 = self.get_temp_close_value(
                'data/'+self.__var1.split('.')[0] + '.xlsx')
            time_close_dict2 = self.get_temp_close_value(
                'data/'+self.__var2.split('.')[0] + '.xlsx')
            time_value1 = time_close_dict1.keys()
            time_value2 = time_close_dict2.keys()
            time_value_in_common = list(
                set(time_value1).intersection(set(time_value2)))
            time_value_in_common = filter(
                self.filter_value_by_time, time_value_in_common)
            time_value_in_common.sort()
            result_list = []
            for item in time_value_in_common:
                result_list.append(
                    round(time_close_dict1[item] - time_close_dict2[item], 2))
            return result_list
    ...
    def cal_longterm_EMA(self):
        ...
        for i in range(len(close_value_list) - 26):
            temp = float(
                close_value_list[i+26]*2/27) + float(result_list[i]*25/27)
    ...
    def cal_shortterm_EMA(self):
        ...
        for i in range(len(close_value_list) - 12):
            temp = float(close_value_list[i+12]) * \
                2/13 + float(result_list[i])*11/13
            result_list.append(temp)
        return result_list
